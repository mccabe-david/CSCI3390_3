  ...
  def LubyMIS(g_in: Graph[Int, Int]): Graph[Int, Int] = {
    ...
    while (remaining_vertices >= 1) {
      ...
      triplet => {
        ...
        else if (triplet.dstAttr != -1 && triplet.dstAttr != 1){
    ...
    val updates_rdd = g_new.aggregateMessages[Int](
      triplet => {
        ...
        else
        {
          ...
          else if (triplet.srcAttr._2 == 1)
          {
            triplet.sendToSrc(0)
            triplet.sendToDst(1)
          }
          else if (triplet.dstAttr._2 == 1)
          {
            triplet.sendToSrc(1)
            triplet.sendToDst(0)
          }
      ...
      val g_almost = Graph(updates_rdd, g_in.edges)
    ...
    val g_final = Graph(remove_neighbors, g_in.edges)
    g = g_final
